...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var possibleAliens = ["Enemy ship 1", "Enemy ship 2", "Enemy ship 3", "Enemy ship 4"]
    ...
    override func didMove(to view: SKView) {
        ...
        bottomLine = SKSpriteNode(imageNamed: "Menu Screen 5")
...
//        gasIcon.zPosition = 2
      ...
      //player.size = CGSize(width: 88, height: 48)
        ...
        timerLabel.position = CGPoint(x: self.frame.size.width / 4.5, y: self.size.height / 150)
       ...
       // starfield.particleScale = CGFloat(random(min: 1, max: 3))
        ...
        actionArray.append(SKAction.run {
            ...
            if self.livesArray.count == 0 {
                self.lifeBar.texture = SKTexture(imageNamed: "Green Life")
            }
            
               if self.livesArray.count == 2 {
                self.lifeBar.texture = SKTexture(imageNamed: "Yellow Life")
            }
            
            if self.livesArray.count == 1 {
                self.lifeBar.texture = SKTexture(imageNamed: "Red Life")
            }
